...
export default function ExpensesClient({ initialExpenses, projects }: ExpensesClientProps) {
  ...
  const onSubmit = async (data: FormData) => {
    ...
    const { error } = await supabase.from('expenses').insert({
      ...data, project_id: data.project_id || null, created_by: user?.id
    })
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-bg-tertiary border-b border-border">
                {['Date', 'Type', 'Description', 'Project', 'Amount'].map(h => (
            ...
            <tbody>
              ...
              {filtered.length === 0 && (
                <tr><td colSpan={5} className="px-4 py-12 text-center text-text-muted text-sm">No expenses found</td></tr>
              )}
            </tbody>
          </table>
        </div>
          ...
          <>
            ...
            >
              <div className="flex items-center justify-between px-6 py-4 border-b border-border">
                ...
                <button onClick={() => setModalOpen(false)} className="w-8 h-8 rounded-lg flex items-center justify-center text-text-muted hover:text-text hover:bg-bg-tertiary transition-all"><X size={16} /></button>
              </div>
              ...
              <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-border">
                ...
                <Button onClick={handleSubmit(onSubmit)} loading={isSubmitting}>Add Expense</Button>
              </div>
            </motion.div>
          </>
        )}
